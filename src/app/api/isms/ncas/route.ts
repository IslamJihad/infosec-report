...
export async function GET() {
  try {
    const items = await db.ismsNca.findMany({
    ...
    return NextResponse.json({ error: "Failed to fetch ncas" }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const item = await db.ismsNca.create({
      data: { workspaceId: ISMS_WORKSPACE_ID, ...payload } as any,
    });

    return NextResponse.json(item);
  } catch (error) {
    console.error("Error creating ncas:", error);
    return NextResponse.json({ error: "Failed to create nca" }, { status: 500 });
  }
}
