...
function parseRiskSeverity(raw: string | null): AnalyticsRiskSeverity | undefined {
...
function parseRiskStatus(raw: string | null): AnalyticsRiskStatus | undefined {
...
function parseGroupBy(raw: string | null): AnalyticsGroupBy {
...
function normalizeDate(raw: string | null): Date | undefined {
...
export async function GET(request: Request) {
  try {
    ...
    const riskSeverityFilter = parseRiskSeverity(url.searchParams.get('riskSeverity'));
    const riskStatusFilter = parseRiskStatus(url.searchParams.get('riskStatus'));
    const reportIdFilter = (url.searchParams.get('reportId') || '').trim();
    ...
    if (from || to) reportWhere.createdAt = createdAtFilter;
    if (reportIdFilter) reportWhere.id = reportIdFilter;
    ...
    orderedReports.forEach((report) => {
      ...
      report.risks.forEach((risk) => {
        ...
        if (riskSeverityFilter && severity !== riskSeverityFilter) return;
        if (riskStatusFilter && status !== riskStatusFilter) return;
